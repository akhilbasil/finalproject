...
const FunnelProductsTable: React.FC<FunnelProductsTableProps> = ({
  ...
  const handleAddProduct = (product: Product) => {
    const productIdExists = liveProducts.find(
      (prod) => prod.productId === product.defaultPrice.id
    )
    productIdExists
      ? setLiveProducts(
          liveProducts.filter((prod) => prod.productId !== product.defaultPrice.id)
        )
      : setLiveProducts([
          ...liveProducts,
          {
            productId: product.defaultPrice.id,
            recurring: product.defaultPrice.recurring,
          },
        ])
  }
        ...
        <TableBody className="font-medium truncate">
          {products.map((product) => (
            <TableRow key={product.id}>
              <TableCell>
                <Input
                  defaultChecked={!!liveProducts.find((prod) => prod.productId === product.defaultPrice.id)}
              ...
              <TableCell>
                {product.defaultPrice.recurring ? 'Recurring' : 'One Time'}
              </TableCell>
              <TableCell className="text-right">
                ${product.defaultPrice.unitAmount / 100}
              </TableCell>
            </TableRow>
          ))}
        </TableBody>
